...
# Home-screen icons for the web version (iPhone "Add to Home Screen" and installable web app)
WEB = pathlib.Path(__file__).resolve().parent.parent / 'www' / 'icons'
WEB.mkdir(parents=True, exist_ok=True)
crystal(180).convert('RGB').save(WEB / 'apple-touch-icon.png')   # iOS adds its own rounded corners
crystal(192).save(WEB / 'icon-192.png')
crystal(512).save(WEB / 'icon-512.png')
m = Image.new('RGBA', (512, 512), BG + (255,)); m.alpha_composite(crystal(512, bg=False, scale=0.7))
m.save(WEB / 'maskable-512.png')
print('web icons written to', WEB)
