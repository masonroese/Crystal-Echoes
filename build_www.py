...
mobile_css = r"""
/* ---------- phone layout ---------- */
html,body{height:100%;margin:0;overflow:hidden;overscroll-behavior:none;background:var(--bg);-webkit-user-select:none;user-select:none;-webkit-tap-highlight-color:transparent;-webkit-touch-callout:none;-webkit-text-size-adjust:100%}
...
<meta name="apple-mobile-web-app-title" content="Crystal Echoes">
...
"""
# offline support for the home-screen web app: register a service worker (skipped inside the Android app)
page = page.replace('</body>', '''<script>
if('serviceWorker' in navigator && location.protocol.startsWith('http') && !(window.Capacitor&&window.Capacitor.isNativePlatform&&window.Capacitor.isNativePlatform()))
...
import json, hashlib
www = root / 'www'
(www / 'manifest.json').write_text(json.dumps({
  "name": "Crystal Echoes", "short_name": "Crystal Echoes",
  "description": "A turn-based 1v1 arena game where every past turn replays as an echo.",
  "start_url": "./", "scope": "./", "display": "fullscreen", "orientation": "landscape",
  "background_color": "#0d1315", "theme_color": "#0d1315",
  "icons": [
    {"src": "icons/icon-192.png", "sizes": "192x192", "type": "image/png"},
    {"src": "icons/icon-512.png", "sizes": "512x512", "type": "image/png"},
    {"src": "icons/maskable-512.png", "sizes": "512x512", "type": "image/png", "purpose": "maskable"}
  ]}, indent=2))
files = ['./', 'index.html', 'manifest.json'] + sorted('fonts/' + f.name for f in (www / 'fonts').glob('*.ttf')) + sorted('icons/' + f.name for f in (www / 'icons').glob('*.png'))
version = hashlib.sha1(page.encode()).hexdigest()[:10]  # new game build -> new cache -> phones pick up the update
(www / 'sw.js').write_text(f'''// Keeps Crystal Echoes playable offline once it has been opened.
const CACHE='crystal-echoes-{version}';
...
''')
print('wrote manifest.json and sw.js, cache', version)
